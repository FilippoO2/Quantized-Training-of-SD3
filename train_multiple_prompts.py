...
def load_text_encoders(class_one, class_two, class_three):
    ...
    text_encoder_one = class_one.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="text_encoder", revision=args.revision, variant=args.variant, torch_dtype = torch.bfloat16
    )
    text_encoder_two = class_two.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="text_encoder_2", revision=args.revision, variant=args.variant,torch_dtype = torch.bfloat16
    )
    text_encoder_three = class_three.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="text_encoder_3", revision=args.revision, variant=args.variant, quantization_config=quantization_config,torch_dtype = torch.bfloat16
    )
...
def main(args):
    ...
    transformer = SD3Transformer2DModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="transformer", revision=args.revision, variant=args.variant,    torch_dtype=torch.bfloat16,
    ...
    pooled_embeded_prompts_list = torch.empty((0, 2048), dtype=torch.bfloat16).to("cpu")

    # Number of arrays in the dataloader
    num_arrays = len(train_dataloader)
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        pooled_embeded_prompts_list = torch.cat((pooled_embeded_prompts_list, pooled_prompt_embeds), dim=0)
        # print(len(pooled_embeded_prompts_list))
        # print(len(pooled_embeded_prompts_list))
        prompt_embeds_list[step] = prompt_embeds

    del prompt_embeds, pooled_prompt_embeds, text_encoder_one, text_encoder_two, text_encoder_three, prompts, text_encoders, tokenizers
    torch.cuda.empty_cache()
    gc.collect()    
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):          
            ...
            with accelerator.accumulate(models_to_accumulate):
                # if "text_encoders" not in globals():
                #     tokenizers = [tokenizer_one, tokenizer_two, tokenizer_three]
                #     text_encoder_one, text_encoder_two, text_encoder_three = load_text_encoders(
                #         text_encoder_cls_one, text_encoder_cls_two, text_encoder_cls_three
                #     )
                ...
                if train_dataset.custom_instance_prompts:
                    prompt_embeds = prompt_embeds_list[step].to(device="cuda:0")
                    pooled_prompt_embeds = pooled_embeded_prompts_list[step].to(device="cuda:0")
